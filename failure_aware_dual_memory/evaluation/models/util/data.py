...
def _ensure_pandarallel():
    """Initialize pandarallel once, with a safe serial fallback."""
    ...
    try:
        ...
        print(f"[Warning] Pandarallel unavailable, falling back to serial graph build: {exc}")
        return False
...
def load_pyg_graphs(
    ...
    if _ensure_pandarallel():
        try:
            graphs = df["cif"].parallel_apply(atoms_to_graph)
        except Exception as exc:
            print(f"[Warning] parallel_apply failed, using serial apply instead: {exc}")
            graphs = df["cif"].apply(atoms_to_graph)
    else:
        graphs = df["cif"].apply(atoms_to_graph)
